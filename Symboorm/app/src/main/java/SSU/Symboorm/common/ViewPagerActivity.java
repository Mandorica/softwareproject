...
public class ViewPagerActivity extends AppCompatActivity {
    ...
    private String rootPath = Util.INSTANCE.getRootPath()+"/DirectTalk9/";
	...
	Button.OnClickListener downloadBtnClickListener = new View.OnClickListener() {
		public void onClick(final View view) {
            if (!Util.INSTANCE.isPermissionGranted((Activity) view.getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                return ;
            }
			...
			FirebaseStorage.getInstance().getReference().child("files/"+message.getMsg()).getFile(localFile).addOnSuccessListener(new OnSuccessListener<FileDownloadTask.TaskSnapshot>() {
				...
				public void onSuccess(FileDownloadTask.TaskSnapshot taskSnapshot) {
					// hideProgressDialog();
					Util.INSTANCE.showMessage(view.getContext(), "Downloaded file");
