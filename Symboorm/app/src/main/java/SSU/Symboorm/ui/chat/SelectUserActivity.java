...
public class SelectUserActivity extends AppCompatActivity {
    ...
    Button.OnClickListener makeRoomClickListener = view -> {
        if (selectedUsers.size() <2) {
            Util.INSTANCE.showMessage(getApplicationContext(), "Please select 2 or more user");
            return;
        }

        selectedUsers.put(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid(), "");
        DocumentReference newRoom = FirebaseFirestore.getInstance().collection("rooms").document();
        CreateChattingRoom(newRoom);
    };

    final Button.OnClickListener addRoomUserClickListener = new View.OnClickListener() {
        public void onClick(View view) {
            if (selectedUsers.size() < 1) {
                Util.INSTANCE.showMessage(getApplicationContext(), "Please select 1 or more user");
                return;
            }
    ...
    public void CreateChattingRoom(final DocumentReference room) {
        ...
        for( String key : selectedUsers.keySet()) {
            ...
            if (title.length() < 20 & !key.equals(uid)) {
                title.append(selectedUsers.get(key)).append(", ");
            }
        }
