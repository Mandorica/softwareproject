...
public class UserPWActivity extends AppCompatActivity {
    ...
    Button.OnClickListener saveBtnClickListener = new View.OnClickListener() {
        public void onClick(final View view) {
            ...
            if (pw1.length()<8) {
                Util.INSTANCE.showMessage(getApplicationContext(), "Please enter at least eight characters.");
                return;
            }
            if (!pw1.equals(user_pw2.getText().toString().trim())) {
                Util.INSTANCE.showMessage(getApplicationContext(), "Password does not match the confirm password.");
                return;
            }
            ...
            user.updatePassword(pw1).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    Util.INSTANCE.showMessage(getApplicationContext(), "Password changed");
